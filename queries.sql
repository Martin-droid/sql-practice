-- SQL practice queries (MySQL syntax)
-- Tables: customers, products, orders — see schema.sql
...
-- 7. For each month of the year 2020, display the number of orders
...
SELECT *
FROM orders
WHERE YEAR(order_date)  = YEAR(CURDATE() - INTERVAL 3 MONTH)
  AND MONTH(order_date) = MONTH(CURDATE() - INTERVAL 3 MONTH);
